...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const cardOffsetsRef = useRef<number[]>([]); // Cache static offsets
  ...
  const updateOffsets = useCallback(() => {
    ...
    // Temporarily clear transforms to get static offsets
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const scaleValue = 1 - scaleProgress * (1 - targetScale);
      ...
      const newTransform = {
        translateY: translateY, // Don't round during calculation for maximum smoothness
        scale: scaleValue,
        rotation: rotation,
        blur: blur
      };
      ...
      if (hasChanged) {
        // High-precision transforms for GPU smoothness
      ...
      if (i === cardsRef.current.length - 1) {
        const isCurrentlyInView = scrollTop >= pinStart && scrollTop <= pinEnd;
        if (isCurrentlyInView && !stackCompletedRef.current) {
          stackCompletedRef.current = true;
          onStackComplete?.();
        } else if (!isCurrentlyInView && stackCompletedRef.current) {
  ...
  useLayoutEffect(() => {
    ...
    cards.forEach((card, i) => {
      ...
      // card.style.webkitTransform = 'translateZ(0)'; // Typescript doesn't like this directly
  ...
  return (
    <div className={`scroll-stack-scroller no-scrollbar ${className}`.trim()} ref={scrollerRef}>
