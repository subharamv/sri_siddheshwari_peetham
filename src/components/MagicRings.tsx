...
export default function MagicRings({
  ...
  propsRef.current = {
    ...
    externalHovered, externalMousePos,
  } as Required<MagicRingsProps>;
  ...
  useEffect(() => {
    ...
    const animate = (t: number) => {
      ...
      const targetMouseX = p.externalMousePos != null ? p.externalMousePos.x : mouseRef.current[0];
      const targetMouseY = p.externalMousePos != null ? p.externalMousePos.y : mouseRef.current[1];
      smoothMouseRef.current[0] += (targetMouseX - smoothMouseRef.current[0]) * 0.08;
      smoothMouseRef.current[1] += (targetMouseY - smoothMouseRef.current[1]) * 0.08;
      const effectiveHovered = p.externalHovered != null ? p.externalHovered : isHoveredRef.current;
      hoverAmountRef.current += ((effectiveHovered ? 1 : 0) - hoverAmountRef.current) * 0.08;
